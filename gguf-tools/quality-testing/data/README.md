...
- `pro-0813`: 100 DeepSeek V4 PRO 0813 continuations with API top-logprob
  slices.
- `flash-vision-exp-openrouter-novita-100`: 100 DeepSeek V4 Flash Vision-Exp
  continuations (text prompts) from OpenRouter's Novita endpoint, greedy, up
  to 64 tokens, reasoning off. That endpoint returned no logprobs. Same 100
  prompts as `flash`, so the two collections can be cross-scored (each GGUF
  against both target sets); the providers differ (official FP8 vs Novita),
  so a `flash`-vs-Vision-Exp NLL difference is not a checkpoint parity
  signal by itself.
