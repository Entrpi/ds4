...
### `flash-vision-exp/images/` (Track B fixtures)

Synthetic, license-clean images regenerated by `images/gen_synthetic.py`
(pure Python, no dependencies): sizes chosen for the Vision-Exp preprocess
(16x16 tiny, 17x9 odd, 532x280 typical, 600x900 portrait, 2000x250 = the 8:1
aspect clamp, 2400x1800 large, 640x480 checker) plus a JPEG twin of the
typical image (JPEG decoder path; the script emits PNG only, the twin was
converted once with `sips -s format jpeg -s formatOptions 85` and is
tracked as-is). They drive the inc-2 encoder bit-exact
gate (`tests/dump_vision_embedding` on the fork vs upstream 110afdd,
`tests/compare_vision_dumps.py`): content does not matter there, only sizes
and formats. Photographic fixtures for the needle-in-image battery are a
