...
/* Slice-parallel replacement for the cluster kernel above, for HC=4, one
 * token, n_embd divisible by 64 (DS4 decode).  DS4_HC_SLICES threadgroups
 * of 256 threads each own 64 columns of all four streams: thread (k, c)
 * expands x[k][c] with the cluster kernel's accumulation order, keeps it
 * in a register, and contributes to the RMS partial and to the 24 gate
 * matvec partials over the slice (dot(w, x*scale) = scale*dot(w, x), so
 * the scale is applied by the last arriver).  Partials go to `partials`
 * [DS4_HC_SLICES][32]; the last-arriving threadgroup sums them in fixed
 * order (deterministic), forms the mixes, the pre/post/comb gate weights,
 * the four-stream collapse and the input norm, exactly as the cluster
 * kernel's tail does, with no spin barrier anywhere.  The `barrier` word
 * is the arrival counter (reset by the last arriver). */
#define DS4_HC_SLICES 64u
kernel void kernel_dsv4_hc_expand4_mix_norm_slices_f16(
        constant ds4_metal_args_hc_norm_mix & args,
        constant ds4_metal_args_dsv4_hc_split_weighted_sum_norm & split_args,
        constant ds4_metal_args_dsv4_hc_expand & ex,
        constant uint & barrier_target,
    ...
    (void)barrier_target;
    (void)completion;
    constexpr uint W = 64u;             // columns per slice
    constexpr uint NROWS = 24u;         // gate matvec rows
    const uint n_embd = (uint)ex.n_embd;
    const uint c = tgpig.x * W + ((uint)tiitg & (W - 1u));   // column
    const uint k = (uint)tiitg / W;                          // stream 0..3

    // Phase A: expand this thread's (stream, column) with the cluster
    // kernel's accumulation order.
    float xv = 0.0f;
    {
        float block_v = *((device const float *)(block_out + c*ex.nb_block0));
        if (ex.has_add) {
            block_v += *((device const float *)(block_add + c*ex.nb_add0));
        }
        const float r0 = *((device const float *)(residual_prev + c*ex.nb_res0 + 0*ex.nb_res1));
        const float r1 = *((device const float *)(residual_prev + c*ex.nb_res0 + 1*ex.nb_res1));
        const float r2 = *((device const float *)(residual_prev + c*ex.nb_res0 + 2*ex.nb_res1));
        const float r3 = *((device const float *)(residual_prev + c*ex.nb_res0 + 3*ex.nb_res1));
        float acc = block_v * *((device const float *)(post + k*ex.nb_post0));
        acc += *((device const float *)(comb + k*ex.nb_comb0 + 0*ex.nb_comb1)) * r0;
        acc += *((device const float *)(comb + k*ex.nb_comb0 + 1*ex.nb_comb1)) * r1;
        acc += *((device const float *)(comb + k*ex.nb_comb0 + 2*ex.nb_comb1)) * r2;
        acc += *((device const float *)(comb + k*ex.nb_comb0 + 3*ex.nb_comb1)) * r3;
        *((device float *)(x + c*ex.nb0 + k*ex.nb1)) = acc;
        xv = acc;
    }

    // Gate matvec partials: simdgroup sg owns rows sg, sg+8, sg+16; lane l
    // owns elements 8l..8l+7 of the 256-element slice (all inside one
    // stream), read as two contiguous half4 against the expanded values
    // kept in threadgroup memory.
    threadgroup float * red = shmem;               // [32] row partials + [24] sumsq slots
    threadgroup float * xs = shmem + 64u;          // [256] expanded values of the slice
    xs[tiitg] = xv;
    const float sq = simd_sum(xv * xv);
    if (tiisg == 0) red[32u + sgitg] = sq;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    {
        const uint e0 = (uint)tiisg * 8u;
        const uint kk = e0 / W;
        const uint cc = e0 - kk * W;
        const uint64_t wstride = (uint64_t)args.n;   // halves per row (4*n_embd)
        device const half * wbase = (device const half *)weight +
            (uint64_t)kk * n_embd + (uint64_t)tgpig.x * W + cc;
        const float x0 = xs[e0 + 0], x1 = xs[e0 + 1], x2 = xs[e0 + 2], x3 = xs[e0 + 3];
        const float x4 = xs[e0 + 4], x5 = xs[e0 + 5], x6 = xs[e0 + 6], x7 = xs[e0 + 7];
        half4 wv[6];
        FOR_UNROLL (uint j = 0; j < 3u; ++j) {
            const uint r = (uint)sgitg + 8u * j;
            device const half4 * wp = (device const half4 *)(wbase + (uint64_t)r * wstride);
            wv[2u*j] = wp[0];
            wv[2u*j + 1u] = wp[1];
        }
        FOR_UNROLL (uint j = 0; j < 3u; ++j) {
            const uint r = (uint)sgitg + 8u * j;
            const half4 a = wv[2u*j], b = wv[2u*j + 1u];
            float acc = (float)a.x * x0;
            acc += (float)a.y * x1;
            acc += (float)a.z * x2;
            acc += (float)a.w * x3;
            acc += (float)b.x * x4;
            ...
            acc += (float)b.z * x6;
            acc += (float)b.w * x7;
            const float pr = simd_sum(acc);
            if (tiisg == 0) red[r] = pr;
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (tiitg < NROWS) {
        partials[tgpig.x * 32u + tiitg] = red[tiitg];
    } else if (tiitg == NROWS) {
        float t = 0.0f;
        for (uint sg = 0; sg < 8u; ++sg) t += red[32u + sg];
        partials[tgpig.x * 32u + NROWS] = t;
    }
    threadgroup_barrier(mem_flags::mem_device_and_threadgroup);

    // Arrival: the last threadgroup finishes the step.
    threadgroup uint *last_flag = (threadgroup uint *)(shmem + 64u + 256u);
    if (tiitg == 0) {
        atomic_thread_fence(mem_flags::mem_device, memory_order_seq_cst, thread_scope_device);
        const uint old = atomic_fetch_add_explicit(barrier, 1u, memory_order_relaxed);
        last_flag[0] = (old + 1u == DS4_HC_SLICES) ? 1u : 0u;
        if (last_flag[0]) {
            atomic_store_explicit(barrier, 0u, memory_order_relaxed);
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (!last_flag[0]) return;

    // Fixed-order reduction of the slice partials: gather all of them into
    // threadgroup memory with every thread first (one wave of independent
    // loads), then sum in slice order from there.
    threadgroup float *tot = shmem + 64u + 256u + 4u;   // [32]
    threadgroup float *gath = tot + 32u + 4u + 8u;      // [DS4_HC_SLICES][NROWS+1]
    atomic_thread_fence(mem_flags::mem_device, memory_order_seq_cst, thread_scope_device);
    for (uint i = (uint)tiitg; i < DS4_HC_SLICES * (NROWS + 1u); i += 256u) {
        const uint g = i / (NROWS + 1u);
        const uint r = i - g * (NROWS + 1u);
        gath[i] = partials[g * 32u + r];
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (tiitg < NROWS + 1u) {
        float t = 0.0f;
        for (uint g = 0; g < DS4_HC_SLICES; ++g) t += gath[g * (NROWS + 1u) + tiitg];
        tot[tiitg] = t;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    const float mean = tot[NROWS] / (float)args.n;
    const float scale = 1.0f / sqrt(mean + args.eps);
    device volatile float *mixes_f32 = (device volatile float *)dst;
    if (tiitg < NROWS && (int)tiitg < args.out_dim) {
        mixes_f32[tiitg] = tot[tiitg] * scale;
    }
    threadgroup_barrier(mem_flags::mem_device_and_threadgroup);

    // Gate weights and collapse, as the cluster kernel's tail.
    threadgroup float *pre_shmem = tot + 32u;    // [4]
    threadgroup float *sum_shmem = pre_shmem + 4u; // [8]
    device float *out = (device float *)split;
    if (tiitg == 0) {
        const float4 pre_z =
            *((device volatile const float4 *)mixes_f32) * hc_scale[0] +
            *((device const float4 *)hc_base);
        const float4 pre = 1.0f / (1.0f + exp(-pre_z)) + split_args.eps;
        *((device float4 *)out) = pre;
        ...
        const float4 post_z =
            ...
            *((device const float4 *)(hc_base + 4));
        *((device float4 *)(out + 4)) = 2.0f / (1.0f + exp(-post_z));
        ds4_hc_comb_weights4_exact(split_args, mixes_f32, hc_scale, hc_base, out);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    // 256 threads, up to 4 float4 each (n_embd <= 4096); larger n_embd loops.
    float4 v[4];
    float ssq = 0.0f;
    for (uint j = 0; j < 4u; ++j) {
        const uint i = (uint)tiitg + j * 256u;
        float4 vv = 0.0f;
        if (i < n4_collapse) {
            vv += x0[i] * pre_shmem[0];
            vv += x1[i] * pre_shmem[1];
    ...
    for (uint i = (uint)tiitg + 4u * 256u; i < n4_collapse; i += 256u) {
        float4 vv = x0[i] * pre_shmem[0];
        vv += x1[i] * pre_shmem[1];
        vv += x2[i] * pre_shmem[2];
        vv += x3[i] * pre_shmem[3];
        ssq += dot(vv, vv);
    ...
    ssq = simd_sum(ssq);
    if (tiisg == 0) sum_shmem[sgitg] = ssq;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    float sumf = 0.0f;
    for (uint sg = 0; sg < 8u; ++sg) sumf += sum_shmem[sg];
    const float norm_scale = rsqrt(sumf / float(split_args.n_embd) + split_args.norm_eps);
    device float4 *dst4 = (device float4 *)collapse_dst;
    device const float4 *w4 = (device const float4 *)norm_weight;
    device float4 *norm4 = (device float4 *)norm_dst;
    for (uint j = 0; j < 4u; ++j) {
